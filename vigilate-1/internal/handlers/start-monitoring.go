	...
	if app.PreferenceMap["monitoring_live"] == "1" {
		...
		for _, x := range servicesToMonitor {
			...
			if x.ScheduleUnit == "d" {
				...
				log.Println("masuk sini 24 paling")
			} else {
				sch = fmt.Sprintf("@every %d%s", x.ScheduleNumber, x.ScheduleUnit)
				log.Println("masuk sini 10s paling")
				log.Println(x.ScheduleNumber, x.ScheduleUnit)
				log.Println(sch)
			}
			...
			log.Println("yearone", yearOne, "scheduling", x.ID)
